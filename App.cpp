...
void SubmitShare(Curl& curl, Share& w, unsigned char* scratchbuf)
{
	...
	try
	{
		...
		//TODO: test share here.

		//cout << "Block: " << str << endl;
		
		//string str = VectorToHexString(blockdata);
		//string ret = curl.TestWork(servers[w.server_id],str);
		
		string ret = curl.TestWork(servers[w.server_id],str);
		//cout << "Return return! " << ret << endl;
	}
	...
	while(!shutdown_now)
	{
		...
		if (timeclock - ticks >= 1000)
		{
			...
			if (ticks-lastprimeprinttime >= 4000)
			{
				...
				cout << fermats/double((ticker()-starttime)*0.001) << " fermats/s, " << gandalfs/double((ticker()-starttime)*0.001) << " gandalfs/s." << endl;
				
				cout << SIize(3*3600.0*totalpersec/((ticker()-starttime)*0.001)) << " TOTAL     ";
				for(uint i=2; i<15; ++i)
				{
					if (chainspersec[i] > 0)
					{
